...
public partial struct ShootingSystem : ISystem
{
    ...
    {
        foreach (var (playerTransform, playerDirection, _) in SystemAPI.Query<RefRO<LocalTransform>, RefRO<Direction>, RefRO<PlayerTag>>())
        {
            foreach (var shooter in SystemAPI.Query<RefRW<BulletSpawner>>())
            {
                if (shooter.ValueRO.fireTimer < SystemAPI.Time.ElapsedTime)
                {
                    Entity bullet = state.EntityManager.Instantiate(shooter.ValueRO.bullet);

                    float3 bulletPosition = playerTransform.ValueRO.Position;
                    float3 bulletDirection = playerDirection.ValueRO.direction;
