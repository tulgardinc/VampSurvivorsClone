...
public partial struct ZweihanderWeaponSystem : ISystem
{
...

        {
            foreach (var (zweihander, cooldown) in SystemAPI.Query<RefRO<ZweihanderWeapon>, RefRW<Cooldown>>())
            {
                if (cooldown.ValueRO.timer <= 0)
                {
                    ...
                    NativeList<DistanceHit> enemiesToHit = new NativeList<DistanceHit>(Allocator.TempJob);
                    collisionWorld.OverlapSphere(playerTransform.ValueRO.Position, zweihander.ValueRO.radius, ref enemiesToHit, CollisionFilter.Default);

                    EntityCommandBuffer.ParallelWriter ecb = ecbSystem.CreateCommandBuffer(state.WorldUnmanaged).AsParallelWriter();

                    ComponentLookup<EnemyTag> enemyLookup = SystemAPI.GetComponentLookup<EnemyTag>();
    ...
    [BurstCompile]
    public partial struct ZweihanderDamageJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            if (math.acos(math.dot(hitDirection, attackDirection)) <= sweepingAngle * (math.PI / 180))
            {
                ...
                {
                    EntityUtilities.EnemyHitWithEffectsParallel(index, ecb, hit.Entity, damage, knockbackAmount, hitDirection);
                }
            }
        }
    }
